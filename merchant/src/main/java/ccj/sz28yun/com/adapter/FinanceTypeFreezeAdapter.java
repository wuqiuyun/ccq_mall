 ...
 */
public class FinanceTypeFreezeAdapter extends ArrayListAdapter<FinanceFreezeBean> {

    public FinanceTypeFreezeAdapter(Context context) {
        super(context);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (null == convertView) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.item_finance_type_freeze, null);
            viewHolder = new ViewHolder(convertView);
            convertView.setTag(viewHolder);
        } else {
            viewHolder = (ViewHolder) convertView.getTag();
        }
        FinanceFreezeBean bean = getItem(position);
