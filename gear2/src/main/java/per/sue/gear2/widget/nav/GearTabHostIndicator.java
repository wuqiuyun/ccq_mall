...
public class GearTabHostIndicator extends HorizontalScrollView {
    ...
    private final OnClickListener mTabClickListener = new OnClickListener() {
        public void onClick(View view) {
            ...
            if (mSelectedTabIndex != newSelected && mTabReselectedListener != null) {
    ...
    private ArrayList<TableHostBean> tabHostList = new ArrayList<>();
    ...
    public GearTabHostIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDrawLines(Canvas canvas){
        ...
        if (mPositionOffset > 0f && mSelectedTabIndex < tabCount - 1) {

            View nextTab = mTabLayout.getChildAt(mSelectedTabIndex + 1);
    ...
    public View getSelectTabView(){
        return mTabLayout.getChildAt(mSelectedTabIndex);
    }


    public void setTabList(ArrayList<TableHostBean> tabHostList) {
        this.tabHostList = tabHostList;
        notifyDataSetChanged();
    }

    public void notifyDataSetChanged() {
        mTabLayout.removeAllViews();
        tabCount = tabHostList.size();
        IconPagerAdapter iconAdapter = null;
        for (int i = 0; i < tabCount; i++) {
            CharSequence title = tabHostList.get(i).getTitle();
            if (title == null) {
                title = EMPTY_TITLE;
            }
            int iconResId = 0;
        ...
        if (mSelectedTabIndex > tabCount) {
            mSelectedTabIndex = tabCount - 1;
        }
        setCurrentItem(mSelectedTabIndex);
        requestLayout();
    }
    ...
    public void setCurrentItem(int item) {
        if (tabHostList == null || tabHostList.size() == 0) {
