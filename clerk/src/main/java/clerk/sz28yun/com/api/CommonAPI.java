...
public class CommonAPI {
    ...
    public Observable<UserBean> login(String username, String password, String imei, String token) {
        ...
        params.put("gtype", "3"); //登录类型 1 商城会员 member 2 卖家 store 3=业务员（运营商） marketer 4=代理商
    ...
    /**
     * 忘记密码
     * @param memberName  用户账号
     * @param memberPwd 用户密码
     * @param code 验证码
     * @param logId 验证码id
     * @return
     */
    public Observable<String> forgetPassword(String memberName, String memberPwd, String code, String logId) {
    ...
    public Observable<ShareConfigBean> getShareConfig(String memberId) {
        ...
        Map<String, String> params = new HashMap<>();
        params.put("member_id", memberId);
        return new ClerkObserver(ApiConnectionFactory.getInstance().createPOST(url, params), new GearParser<ShareConfigBean>(ShareConfigBean.class))
                .observeOnMainThread();
    }

    public Observable<StatisticBean> getStatistic(String memberId) {
        String url = GlobalDataStorageCache.getInstance().getConfigData().API_DOMAIN + BaseURL.URL_STATISTIC;
        Map<String, String> params = new HashMap<>();
        params.put("member_id", memberId);
        return new ClerkObserver(ApiConnectionFactory.getInstance().createPOST(url, params), new GearParser<StatisticBean>(StatisticBean.class))
                .observeOnMainThread();
    }
